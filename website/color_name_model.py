...
def download_model_to_local(model_name, local_path):
    """
    下载模型到本地目录，避免重复下载
    
    Args:
        model_name: 模型名称，如 'bert-base-uncased'
        local_path: 本地保存路径
    ...
    try:
        ...
        if os.path.exists(local_path):
            ...
            pytorch_file = os.path.join(local_path, "pytorch_model.bin")
            
            if os.path.exists(config_file) and (os.path.exists(safetensors_file) or os.path.exists(pytorch_file)):
                print(f"模型 {model_name} 已存在于 {local_path}，跳过下载")
                return True
        ...
        # 下载模型，优先使用safetensors格式
        try:
            model = AutoModel.from_pretrained(model_name, use_safetensors=True)
            model.save_pretrained(local_path, use_safetensors=True)
            print(f"✅ 成功下载模型 {model_name} (safetensors格式)")
        except Exception as e:
            print(f"safetensors格式下载失败: {e}")
            print("尝试下载标准格式...")
            model = AutoModel.from_pretrained(model_name)
            model.save_pretrained(local_path)
            print(f"✅ 成功下载模型 {model_name} (标准格式)")
...
class NameEncoder(nn.Module):
    ...
    def __init__(self, model_name='bert-base-uncased', emb_dim=64, max_length=32, 
        ...
        if local_model_path:
            ...
            if not os.path.exists(local_model_path) or not os.path.exists(os.path.join(local_model_path, "config.json")):
                logger.info("本地模型不存在或不完整，开始下载...")
                if not download_model_to_local(model_name, local_model_path):
                    logger.error("模型下载失败，将使用在线模式")
        ...
        if local_model_path and os.path.exists(local_model_path):
            ...
            if os.path.exists(safetensors_file):
                logger.info("检测到safetensors文件，使用safetensors格式加载...")
                try:
                    ...
                    logger.error(f"safetensors加载失败: {e}")
                    raise e
            elif os.path.exists(pytorch_file):
                logger.info("检测到pytorch_model.bin文件，使用标准格式加载...")
                try:
                    self.pretrained_model = AutoModel.from_pretrained(local_model_path)
                    logger.info("✅ 成功加载模型（标准格式）")
                except Exception as e:
                    logger.error(f"标准格式加载失败: {e}")
                    raise e
            else:
                logger.error(f"本地模型目录中未找到有效的模型文件: {local_model_path}")
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"safetensors下载失败: {e1}")
                    try:
                        self.pretrained_model = AutoModel.from_pretrained(model_name)
                        logger.info("成功从网络下载模型（标准格式）")
                    except Exception as e2:
                        logger.info(f"标准下载失败: {e2}")
                        # 最后尝试
                        self.pretrained_model = AutoModel.from_pretrained(model_name)
